...
//как считается сумма коллектед в примере??
// для примера: gems x2 ценность, поэтому ценность одного гема 23*2=46
//итого идем по алгоритму 46*1+46*2+gold(ценность 11)*4 = 182
// как получилось 93? 
//также (строка 533), в условии прописано:
//Бот идёт в смежную комнату с наименьшим номером. Если все смежные комнаты уже
//посещены, он идёт в ближайшую не посещённую комнату с наименьшим номером.
//а что делать если все комнаты уже посещены? я короче этот случай проверяю и для него
//запускаю вторую стадию, если еды больше чем надо на обратный путь то с этим условием:
...
bool isCorrectFirstLine(const std::string& line) {
...
bool isCorrectLastLine(const std::string& line) {
    std::stringstream ss(line);
    std::string food;
    std::string resource;
    std::string extra;
    ...
    int foodValue = std::stoi(food);
    if(foodValue < 2 || foodValue > 255) {
        return false;
    }
...
int FindtheBiggestValueIndex(std::vector<int> RoomSources, std::vector<int> values, int indexOfDoubleValue) {
    if(RoomSources[indexOfDoubleValue] > 0) {
